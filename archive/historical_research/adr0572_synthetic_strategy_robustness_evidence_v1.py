...
def _build_synthetic_strategy_robustness_evidence_v1(
    ...
    *,
    ...
    source_receipt = verify_synthetic_strategy_report_bundle_v1(source_bundle)
    ...
    if run_capture is not None and len(run_capture) != executed_run_count:
    ...
    bundle = {
        ...
        "completed_evidence": list(_COMPLETED_EVIDENCE),
        "gaps": list(_GAPS),
...
def _verify_bundle(bundle: dict[str, Any]) -> None:
    ...
    if bundle.get("plan") != plan_synthetic_strategy_robustness_evidence_v1():
    ...
    if verify_synthetic_strategy_report_bundle_v1(source_bundle).get("status") != "PASS":
    ...
    if bundle.get("completed_evidence") != list(_COMPLETED_EVIDENCE):
        raise SyntheticStrategyRobustnessError("completed evidence mismatch")
    if bundle.get("gaps") != list(_GAPS):
...
def render_synthetic_strategy_robustness_markdown_v1(bundle: dict[str, Any]) -> str:
    receipt = verify_synthetic_strategy_robustness_evidence_v1(bundle)
