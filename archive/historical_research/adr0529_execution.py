...
class ResearchExecutionSimulator:
    ...
    def submit_order(self, order: Order, portfolio: Portfolio) -> Fill:
        if order.action not in {Action.BUY, Action.SELL}:
            raise ValueError("ResearchExecutionSimulator only accepts BUY and SELL orders.")
        numeric = {
        ...
        try:
            ...
            raise ValueError("ResearchExecutionSimulator requires numeric order, account, fee, and slippage values.") from exc
        if any(not math.isfinite(value) for value in parsed.values()):
            raise ValueError("ResearchExecutionSimulator requires finite order, account, fee, and slippage values.")
        ...
        if not 0 <= fee_rate < 1 or not 0 <= slippage_pct < 1:
            raise ValueError("ResearchExecutionSimulator fee and slippage must be in [0, 1).")
        if (
            order_price <= 0
            or order_quantity <= 0
            or cash < 0
            ...
            raise ValueError("ResearchExecutionSimulator received an invalid order or account state.")
        fill_price = order_price * (1 + slippage_pct if order.action == Action.BUY else 1 - slippage_pct)
        if not math.isfinite(fill_price) or fill_price <= 0:
            raise ValueError("ResearchExecutionSimulator execution price is invalid.")
        pnl = 0.0
        ...
        if order.action == Action.BUY:
            ...
            if not math.isfinite(all_in_unit_cost) or all_in_unit_cost <= 0:
                raise ValueError("ResearchExecutionSimulator derived an invalid all-in unit cost.")
            filled_quantity = min(order_quantity, cash / all_in_unit_cost)
            if filled_quantity <= 0:
                raise ValueError("ResearchExecutionSimulator cannot fill a buy order without available cash.")
            ...
            if any(not math.isfinite(value) for value in prospective) or new_cash < -1e-9:
                raise ValueError("ResearchExecutionSimulator buy fill would create an invalid account state.")
            ...
            if filled_quantity <= 0:
                raise ValueError("ResearchExecutionSimulator cannot fill a sell order without an open position.")
            notional = filled_quantity * fill_price
            fee = notional * fee_rate
            ...
            if any(not math.isfinite(value) for value in prospective) or new_cash < -1e-9:
                raise ValueError("ResearchExecutionSimulator sell fill would create an invalid account state.")
