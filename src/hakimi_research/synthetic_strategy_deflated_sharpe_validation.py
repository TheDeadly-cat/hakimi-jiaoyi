...
PLAN_SCHEMA_VERSION = "synthetic-strategy-deflated-sharpe-validation-plan-v1"
BUNDLE_SCHEMA_VERSION = "synthetic-strategy-deflated-sharpe-validation-bundle-v1"
RECORD_SCHEMA_VERSION = "synthetic-strategy-deflated-sharpe-validation-record-v1"
RECEIPT_SCHEMA_VERSION = "synthetic-strategy-deflated-sharpe-validation-receipt-v1"
REFERENCE_PLAN_SCHEMA_VERSION = (
    "synthetic-strategy-deflated-sharpe-validation-plan-v2"
)
REFERENCE_BUNDLE_SCHEMA_VERSION = (
    "synthetic-strategy-deflated-sharpe-validation-bundle-v2"
)
REFERENCE_RECEIPT_SCHEMA_VERSION = (
    "synthetic-strategy-deflated-sharpe-validation-receipt-v2"
)
...
class SyntheticStrategyDeflatedSharpeValidationError(ValueError):
    pass


def _fail(path: str, message: str) -> None:
    raise SyntheticStrategyDeflatedSharpeValidationError(f"{path}: {message}")
...
def plan_synthetic_strategy_deflated_sharpe_validation_v1() -> dict[str, Any]:
...
def plan_synthetic_strategy_deflated_sharpe_validation_v2() -> dict[str, Any]:
    source_plan = plan_synthetic_strategy_trial_return_matrix_v2()
    plan = plan_synthetic_strategy_deflated_sharpe_validation_v1()
...
def _build_record(source_record: dict[str, Any]) -> dict[str, Any]:
    ...
    diagnostic = build_deflated_sharpe_diagnostic(matrix)
    receipt = verify_deflated_sharpe_diagnostic(diagnostic, matrix)
    record = {
        ...
        "deflated_sharpe_diagnostic": diagnostic,
        "deflated_sharpe_receipt": receipt,
...
def _build_synthetic_strategy_deflated_sharpe_validation(
    ...
    *,
    ...
    if type(execute) is not bool or execute is not True:
        raise SyntheticStrategyDeflatedSharpeValidationError(
    ...
    bundle = {
        ...
        "runtime_mutations": False,
        "computed_diagnostics": ["DEFLATED_SHARPE_DIAGNOSTIC_COMPUTED"],
        "gaps": _gaps(),
        "authority": _authority(),
    }
    ...
    if reference_binding:
        verify_synthetic_strategy_deflated_sharpe_validation_v2(bundle)
    else:
        verify_synthetic_strategy_deflated_sharpe_validation_v1(bundle)
    return bundle


def build_synthetic_strategy_deflated_sharpe_validation_v1(
    matrix_bundle: dict[str, Any], *, execute: bool = False
) -> dict[str, Any]:
    return _build_synthetic_strategy_deflated_sharpe_validation(
        matrix_bundle,
        execute=execute,
        plan=plan_synthetic_strategy_deflated_sharpe_validation_v1(),
...
def build_synthetic_strategy_deflated_sharpe_validation_v2(
    matrix_bundle: dict[str, Any], *, execute: bool = False
) -> dict[str, Any]:
    return _build_synthetic_strategy_deflated_sharpe_validation(
        matrix_bundle,
        execute=execute,
        plan=plan_synthetic_strategy_deflated_sharpe_validation_v2(),
...
def _verify_record(record: dict[str, Any], source_record: dict[str, Any]) -> None:
    ...
    receipt = verify_deflated_sharpe_diagnostic(
        record["deflated_sharpe_diagnostic"], matrix
    )
    if receipt != record["deflated_sharpe_receipt"]:
        _fail("strategy_record.deflated_sharpe_receipt", "receipt mismatch")
    if (
...
def verify_synthetic_strategy_deflated_sharpe_validation_v1(
    bundle: dict[str, Any],
) -> dict[str, Any]:
    ...
    if bundle["plan"] != plan_synthetic_strategy_deflated_sharpe_validation_v1():
    ...
    if bundle["computed_diagnostics"] != ["DEFLATED_SHARPE_DIAGNOSTIC_COMPUTED"]:
...
def verify_synthetic_strategy_deflated_sharpe_validation_v2(
    bundle: dict[str, Any],
) -> dict[str, Any]:
    canonical_trial_return_matrix_sha256(bundle)
    expected_keys = {
    ...
    if bundle["computed_diagnostics"] != [
        "DEFLATED_SHARPE_DIAGNOSTIC_COMPUTED"
    ]:
    ...
    if (
        ...
        _fail("bundle.plan.policy", "formal inference escalation")
    if (
        bundle["evidence_state"] != EVIDENCE_STATE
        or bundle["status"] != STATUS
        or bundle["maturity"] != MATURITY
...
def replay_synthetic_strategy_deflated_sharpe_validation_v1(
    bundle: dict[str, Any],
) -> dict[str, Any]:
    receipt = verify_synthetic_strategy_deflated_sharpe_validation_v1(bundle)
    replayed = build_synthetic_strategy_deflated_sharpe_validation_v1(
...
def replay_synthetic_strategy_deflated_sharpe_validation_v2(
    bundle: dict[str, Any],
) -> dict[str, Any]:
    receipt = verify_synthetic_strategy_deflated_sharpe_validation_v2(
        bundle
    )
    replayed = build_synthetic_strategy_deflated_sharpe_validation_v2(
        bundle["source_matrix_bundle"],
        execute=True,
    )
    if replayed != bundle:
...
def _render_synthetic_strategy_deflated_sharpe_validation_markdown(
    bundle: dict[str, Any],
    *,
    ...
    rows = [
        "| Strategy | Registered trials | Implied independent trials | Descriptive DSR probability |",
        "| --- | ---: | ---: | ---: |",
    ]
    for record in bundle["strategy_records"]:
        diagnostic = record["deflated_sharpe_diagnostic"]
        rows.append(
            f"| {record['strategy_id']} | {diagnostic['trial_count']} | "
            f"{diagnostic['effective_independent_trial_count']} | "
            f"{diagnostic['deflated_sharpe_probability']} |"
        )
    markdown = "\n".join(
        [
            title,
            ...
            "- Consumes the verified frozen-stability trial return matrix.",
            "- Additional backtest runs: 0",
            "- All three preregistered candidates remain in each strategy family.",
            "",
            "## GAP",
            *[f"- {gap}" for gap in receipt["gaps"]],
            ...
            "- DSR values are descriptive synthetic diagnostics without a decision threshold.",
            "",
            "## PERMISSION",
...
def render_synthetic_strategy_deflated_sharpe_validation_markdown_v1(
    bundle: dict[str, Any],
) -> str:
    return _render_synthetic_strategy_deflated_sharpe_validation_markdown(
        bundle,
        verifier=verify_synthetic_strategy_deflated_sharpe_validation_v1,
        title="# Synthetic Strategy Deflated Sharpe Diagnostic v1",
    )


def render_synthetic_strategy_deflated_sharpe_validation_markdown_v2(
    bundle: dict[str, Any],
) -> str:
    return _render_synthetic_strategy_deflated_sharpe_validation_markdown(
        bundle,
        verifier=verify_synthetic_strategy_deflated_sharpe_validation_v2,
        title="# Synthetic Strategy Deflated Sharpe Diagnostic v2",
    )


def build_default_synthetic_strategy_deflated_sharpe_validation_v1(
    *, execute: bool = False
) -> dict[str, Any]:
    if type(execute) is not bool or execute is not True:
        raise SyntheticStrategyDeflatedSharpeValidationError(
    ...
    return build_synthetic_strategy_deflated_sharpe_validation_v1(
        matrix_bundle, execute=True
    )


def build_default_synthetic_strategy_deflated_sharpe_validation_v2(
    *,
    execute: bool = False,
    ...
    return build_synthetic_strategy_deflated_sharpe_validation_v2(
        matrix_bundle,
        execute=True,
    )
