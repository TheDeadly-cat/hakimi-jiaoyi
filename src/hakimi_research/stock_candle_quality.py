...
STOCK_CANDLE_QUALITY_CONTRACT_VERSION = "stock-candle-quality-v6"
STOCK_CANDLE_TEMPORAL_CONTRACT_VERSION = "stock-candle-temporal-conformance-v3"
...
def _temporal_conformance(
    ...
    *,
    ...
    daily = context["interval"] in {"1d", "1dutc"}
    calendar_name = (
        infer_market_calendar(context["symbol"], source=context["source"])
        if context["valid"]
        else "UNVERIFIED"
    )
    row_dates = [
        row["date"]
        for row in rows
        if type(row.get("date")) is str and row.get("date")
    ]
    observed_dates = [
        row["date"]
        for row in analysis_eligible_rows
        if type(row.get("date")) is str and row.get("date")
    ]
    calendar_conformance: dict[str, Any] = {}
    calendar_complete = not daily
    if daily:
        if type(schedule_attestation) is not dict:
            calendar_conformance = {
                "schema_version": MARKET_CALENDAR_SCHEMA_VERSION,
                "status": "BLOCK",
                "blockers": ["schedule_attestation_required"],
                "calendar_name": calendar_name,
                "start": min(row_dates) if row_dates else "",
                "end": max(row_dates) if row_dates else "",
                "missing_dates": [],
                "unexpected_dates": [],
            ...
            calendar_conformance = {
                "schema_version": MARKET_CALENDAR_SCHEMA_VERSION,
                "status": "BLOCK",
                "blockers": ["calendar_observation_range_required"],
                "calendar_name": calendar_name,
                "start": "",
                "end": "",
                "missing_dates": [],
            ...
            calendar_conformance = build_market_calendar_contract(
                calendar_name=calendar_name,
        ...
        calendar_complete = calendar_conformance.get("status") == "PASS"
    complete = (
        context["valid"]
        and bool(rows)
    ...
    return {
        ...
        "calendar_required": daily,
        "calendar_name": calendar_name,
        "calendar_status": (
            calendar_conformance.get("status", "BLOCK")
            if daily
            else "NOT_APPLICABLE"
        ),
        "calendar_blockers": list(calendar_conformance.get("blockers") or []),
        "calendar_conformance": calendar_conformance,
        "exchange_calendar_attested": (
            daily
            and type(schedule_attestation) is dict
            and calendar_complete
        ),
        "authority": dict(AUTHORITY_LOCK),
...
def analyze_stock_candle_series(
    ...
    *,
    ...
    if not temporal_complete:
        issue_count = int(temporal_conformance["invalid_row_count"]) + len(
            temporal_conformance.get("calendar_blockers") or []
        )
        warnings.append(
            f"{issue_count} candle temporal/calendar conformance issue(s) detected."
        )
    if latest_break:
        warnings.append(
