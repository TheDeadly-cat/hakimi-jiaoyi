...
from hakimi_research.source_layout import LEGACY_PROJECT_ROOT

TERMINAL_CONFIG_SCHEMA_VERSION = "terminal-config-v1"
ROOT_DIR = LEGACY_PROJECT_ROOT / "exchange_terminal"
WORKSPACE_DIR = ROOT_DIR.parents[2]


def load_local_ai_env() -> bool:
    explicit_load = str(os.getenv("HAKIMI_LOAD_LOCAL_AI_ENV") or "").strip().lower() in {
        "1", "true", "yes", "on",
    }
    if not explicit_load:
        return False
    skip_local_env = any(
...
APP_NAME = "哈基米研究"
TERMINAL_VERSION = "v2"
TERMINAL_RELEASE_NAME = "哈基米研究 v2"
...
__all__ = [
    ...
    *sorted(
        name
        for name in globals()
        if name.isupper() and name != "LEGACY_PROJECT_ROOT"
    ),
]
