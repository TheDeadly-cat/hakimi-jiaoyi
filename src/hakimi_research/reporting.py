"""Deterministic, fail-closed JSON report persistence for research runs."""
...
def save_json_report(
    ...
    *,
    ...
    """Persist canonical JSON once; identical retries are idempotent."""
    ...
    try:
        ...
        if report_path.read_text(encoding="utf-8") != rendered:
            raise FileExistsError(
                f"report path already contains different content: {report_path}"
            ) from None
...
def save_json_report_bundle_v2(
    ...
    try:
        ...
        if report_path.read_text(encoding="utf-8") != rendered:
            raise FileExistsError(
                f"report bundle path already contains different content: {report_path}"
            ) from None
    return str(report_path)
