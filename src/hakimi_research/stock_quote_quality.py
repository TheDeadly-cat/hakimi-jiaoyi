...
STOCK_QUOTE_QUALITY_CONTRACT_VERSION = "stock-quote-quality-v4"
STOCK_QUOTE_BOOK_COHERENCE_VERSION = "stock-quote-book-coherence-v1"
...
def normalize_stock_quote_quality(
    ...
    *,
    ...
    numeric_inputs = {
        ...
        "open24h": normalized.get("open24h"),
        "high24h": normalized.get("high24h"),
        "low24h": normalized.get("low24h"),
        "bidPx": normalized.get("bidPx"),
        "askPx": normalized.get("askPx"),
    }
    ...
    numeric_inputs["previous_close"] = previous_value
    numeric_inputs["provider_change"] = (
        provider_change
        if provider_change is not None
        else normalized.get("change24h_pct")
    )
    numeric_coercion_fields = sorted(
        field for field, value in numeric_inputs.items() if type(value) is str
    )
    ...
    book_midpoint = (bid + ask) / 2 if book_status == "VALID" else 0.0
    book_spread = ask - bid if book_status == "VALID" else 0.0
    daily_range = high - low if high > 0 and low > 0 and high >= low else 0.0
    book_coherence_issues: list[str] = []
    if book_status == "CROSSED":
        book_coherence_issues.append("CROSSED_BOOK")
    elif book_status == "ONE_SIDED":
        book_coherence_issues.append("ONE_SIDED_BOOK")
    elif book_status == "NOT_SUPPLIED":
        book_coherence_issues.append("BOOK_NOT_SUPPLIED")
    elif high <= 0 or low <= 0 or high < low:
        book_coherence_issues.append("DAILY_PRICE_ENVELOPE_UNAVAILABLE")
    else:
        if book_midpoint < low or book_midpoint > high:
            book_coherence_issues.append("BOOK_MIDPOINT_OUTSIDE_DAILY_ENVELOPE")
        if daily_range <= 0:
            book_coherence_issues.append("DAILY_PRICE_RANGE_ZERO")
        elif book_spread > daily_range:
            book_coherence_issues.append("BOOK_SPREAD_EXCEEDS_DAILY_RANGE")

    severe_book_issues = {
        "BOOK_MIDPOINT_OUTSIDE_DAILY_ENVELOPE",
        "BOOK_SPREAD_EXCEEDS_DAILY_RANGE",
    }.intersection(book_coherence_issues)
    if severe_book_issues:
        book_message = "盘口中点或价差与日内价格区间不一致"
        if scope_value == "REGULAR":
            quarantine_reasons.append(book_message)
        else:
            warnings.append(f"{book_message}，且盘口时段口径未证明一致")
    if "DAILY_PRICE_RANGE_ZERO" in book_coherence_issues:
        warnings.append("日内价格区间为零，盘口价差完整性无法验证")
    book_coherence_status = (
        "PASS"
        if not book_coherence_issues
        else "REVIEW"
        if book_status == "CROSSED" or (scope_value == "REGULAR" and severe_book_issues)
        else "DEGRADED"
    )
    ...
    if numeric_coercion_fields:
        warnings.append(
            f"{len(numeric_coercion_fields)} 个报价数值字段经过字符串转换，完整性不予确认"
        )
    ...
    quote_complete = (
        ...
        and book_coherence_status == "PASS"
        and not numeric_coercion_fields
    ...
    quality = {
        ...
        "book_coherence_version": STOCK_QUOTE_BOOK_COHERENCE_VERSION,
        "book_coherence_status": book_coherence_status,
        "book_coherence_issues": book_coherence_issues,
        "book_midpoint": round(book_midpoint, 6) if book_midpoint > 0 else 0.0,
        "book_spread": round(book_spread, 6) if book_spread >= 0 else 0.0,
        "daily_price_range": round(daily_range, 6) if daily_range >= 0 else 0.0,
        "numeric_input_status": "COERCED" if numeric_coercion_fields else "NATIVE",
        "numeric_coercion_fields": numeric_coercion_fields,
