...
CONTRACT_VERSION = "deterministic-strategy-statistical-correction-benchmark-v2"
VERIFIER_VERSION = (
    "deterministic-strategy-statistical-correction-benchmark-verifier-v2"
)
MANIFEST_VERSION = (
    "deterministic-strategy-statistical-correction-benchmark-manifest-v2"
)
RECEIPT_VERSION = (
    "deterministic-strategy-statistical-correction-benchmark-receipt-v2"
)
MATURITY = "SYNTHETIC_STATISTICAL_CORRECTION_AND_BOOTSTRAP_ONLY"
REFERENCE_ROOT = (
    REPOSITORY_ROOT
    / "examples"
    / "deterministic_strategy_statistical_correction_benchmark_v2"
)
...
SOURCE_RELATIVE_PATHS = (
    "src/hakimi_research/deterministic_strategy_statistical_correction_benchmark_v2.py",
...
_FORBIDDEN_RECEIPT_TOKENS = (
    ...
    b"strategy_records",
    b"bootstrap_evidence",
    b"confidence_intervals",
    b"source_matrix_bundle",
    b"source_robustness_bundle",
)


class DeterministicStrategyStatisticalCorrectionBenchmarkV2Error(ValueError):
    pass
...
def _reference_root(value: str | None) -> Path:
    ...
    if type(value) is not str or not value or value != value.strip():
        raise DeterministicStrategyStatisticalCorrectionBenchmarkV2Error(
...
def _require_receipt(
    ...
    *,
    ...
    if type(receipt) is not dict or receipt.get("status") != status:
        raise DeterministicStrategyStatisticalCorrectionBenchmarkV2Error(
...
def _build_compact_receipt() -> dict[str, Any]:
    ...
    bootstrap_bundle = build_synthetic_strategy_bootstrap_validation_v2(
        source_bundle,
        execute=True,
    )
    bootstrap_receipt = verify_synthetic_strategy_bootstrap_validation_v2(
        bootstrap_bundle,
        source_bundle,
    )
    _require_receipt(bootstrap_receipt, status="BLOCK", label="bootstrap")
    if bootstrap_receipt.get("state") != "OBSERVED":
        raise DeterministicStrategyStatisticalCorrectionBenchmarkV2Error(
            "bootstrap_observation_state_invalid"
        )
    bootstrap_replay = replay_synthetic_strategy_bootstrap_validation_v2(
        bootstrap_bundle,
        source_bundle,
    )
    if bootstrap_replay.get("replay_status") != "EXACT_MATCH":
        raise DeterministicStrategyStatisticalCorrectionBenchmarkV2Error(
            "bootstrap_replay_failed"
        )
    ...
    tie_bounds_receipt = verify_synthetic_strategy_cscv_pbo_tie_bounds_v2(
        tie_bounds_bundle
    )
    _require_receipt(
        tie_bounds_receipt,
        status="BLOCK",
        label="cscv_pbo_tie_bounds",
    )
    if tie_bounds_receipt.get("state") != "OBSERVED_WITH_GAPS":
        raise DeterministicStrategyStatisticalCorrectionBenchmarkV2Error(
    ...
    stage_gaps = {
        ...
        "bootstrap": list(bootstrap_receipt["gaps"]),
    }
    receipt_core = {
        ...
        "bootstrap_source_dependency_bound_run_count": bootstrap_receipt[
            "source_dependency_bound_run_count"
        ],
        "bootstrap_source_git_bound_run_count": bootstrap_receipt[
            "source_git_bound_run_count"
        ],
        ...
        "statistical_analysis_run_count": (
            ...
            + bootstrap_receipt["executed_run_count"]
        ),
        "additional_backtest_run_count": (
            ...
            + bootstrap_receipt["additional_backtest_run_count"]
        ),
        "formal_inference_claimed": False,
        "decision_threshold": None,
        "bootstrap_formal_inference_claimed": bootstrap_receipt[
            "formal_inference_claimed"
        ],
        "bootstrap_decision_threshold": bootstrap_receipt[
            "decision_threshold"
        ],
        "runtime_mutations": False,
        "stage_gaps": stage_gaps,
        "remaining_gaps": sorted(
            {gap for gaps in stage_gaps.values() for gap in gaps}
        ),
        "authority": dict(tie_bounds_receipt["authority"]),
        "claims": dict(_CLAIMS),
    }
    if not all(
        value is False for value in receipt_core["authority"].values()
    ) or not all(
        value is False for value in bootstrap_receipt["authority"].values()
    ):
        raise DeterministicStrategyStatisticalCorrectionBenchmarkV2Error(
...
def _render_receipt_markdown(receipt: dict[str, Any]) -> str:
    lines = [
        "# Deterministic Synthetic Statistical Correction and Bootstrap Benchmark",
        "",
        "## SOURCE",
        "",
        f"- Source bundle SHA-256: `{receipt['source_bundle_sha256']}`",
        f"- Robustness bundle SHA-256: `{receipt['robustness_bundle_sha256']}`",
        f"- Trial matrix bundle SHA-256: `{receipt['trial_matrix_bundle_sha256']}`",
        f"- Deflated Sharpe bundle SHA-256: `{receipt['deflated_sharpe_bundle_sha256']}`",
        f"- CSCV-PBO bundle SHA-256: `{receipt['cscv_pbo_bundle_sha256']}`",
        f"- Tie-bounds bundle SHA-256: `{receipt['cscv_pbo_tie_bounds_bundle_sha256']}`",
        f"- Bootstrap bundle SHA-256: `{receipt['bootstrap_bundle_sha256']}`",
        (
            "- Reconstructed runs: "
        ...
        f"- Dependency-bound runs: {receipt['total_dependency_bound_run_count']}.",
        "- Statistical correction and Bootstrap backtest runs: 0",
        "",
        "## GAP",
        "",
        *[f"- `{gap}`" for gap in receipt["remaining_gaps"]],
        ...
        (
            "- Bootstrap coverage: "
            f"{receipt['bootstrap_observed_evidence_count']} observed strategy records, "
            f"{receipt['bootstrap_paired_observation_count_per_strategy']} paired "
            "observations per strategy."
        ),
        "- No DSR probability, PBO rate, Bootstrap interval value, or threshold is stored here.",
        "- No formal inference or parameter selection is authorized.",
        "",
        "## PERMISSION",
        "",
...
def build_deterministic_strategy_statistical_correction_reference_material_v2() -> (
    dict[str, Any]
):
...
def _verify_reference_material(
    root: Path,
    material: dict[str, Any],
) -> dict[str, Any]:
    if type(material) is not dict:
        raise DeterministicStrategyStatisticalCorrectionBenchmarkV2Error(
            "reference_material_exact_dict_required"
        )
    if not root.is_dir():
        raise DeterministicStrategyStatisticalCorrectionBenchmarkV2Error(
            "reference_root_missing"
        )
    file_names = sorted(path.name for path in root.iterdir() if path.is_file())
    ...
    if type(expected_receipt) is not dict or type(expected_manifest) is not dict:
        raise DeterministicStrategyStatisticalCorrectionBenchmarkV2Error(
            "reference_json_exact_object_required"
        )
    receipt_core = {
        key: value
    ...
    checks = {
        ...
        and manifest["source_file_count"] == len(SOURCE_RELATIVE_PATHS),
        "all_source_runs_dependency_bound": receipt[
            "total_dependency_bound_run_count"
        ]
        == 179,
        "git_gap_retained": receipt["git_bound_run_count"] == 0
        and receipt["bootstrap_source_git_bound_run_count"] == 0
        ...
        "bootstrap_source_consistent": receipt[
            "bootstrap_source_bundle_sha256"
        ]
        == receipt["source_bundle_sha256"],
        "bootstrap_scope_bound": receipt[
            "bootstrap_observed_evidence_count"
        ]
        == 6
        and receipt["bootstrap_gap_evidence_count"] == 0
        and receipt["bootstrap_paired_observation_count_per_strategy"] == 169
        and receipt["bootstrap_replicate_count"] == 1000
        and receipt["bootstrap_interval_count_per_strategy"] == 3
        and receipt["bootstrap_source_dependency_bound_run_count"] == 32
        and receipt["bootstrap_replay_status"] == "EXACT_MATCH",
        ...
        and receipt["bootstrap_formal_inference_claimed"] is False
        and receipt["bootstrap_decision_threshold"] is None
    ...
    return {
        ...
        "bootstrap_replay_status": receipt["bootstrap_replay_status"],
...
def verify_deterministic_strategy_statistical_correction_reference_v2(
    reference_root: str | None = None,
) -> dict[str, Any]:
    root = _reference_root(reference_root)
    material = build_deterministic_strategy_statistical_correction_reference_material_v2()
    return _verify_reference_material(root, material)
