...
REFERENCE_SCHEMA_VERSION = "synthetic-strategy-report-bundle-v2"
REFERENCE_CONTEXT_SCHEMA_VERSION = "synthetic-strategy-reference-context-v1"
...
def _run_backtest(
    *,
    ...
    context = deepcopy(experiment_context) if type(experiment_context) is dict else {}
    context.update({
        "evaluation_role": (
            evaluation_role
            if manifest_evaluation_role is None
            else manifest_evaluation_role
        ),
        "evaluation_protocol_hash": protocol_sha256,
        "evaluation_protocol_verified": experiment_context is not None,
    })
    context.setdefault("git_commit_sha", "")
    context.setdefault("git_worktree_clean", False)
    context.setdefault("dependency_lock_name", "")
    context.setdefault("dependency_lock_hash", "")
    context.setdefault("dependency_lock_fully_pinned", False)
            ...
            manifest_evaluation_role=(
                {
                    ...
                    "FROZEN_COST_1X": "FROZEN_TEST",
                    "FROZEN_COST_2X": "FROZEN_TEST",
                    "FROZEN_COST_3X": "FROZEN_TEST",
                }[role]
...
def _build_benchmarks(
    ...
    for benchmark_id, factory in factories.items():
        benchmarks[benchmark_id] = _run_backtest(
            ...
            experiment_context=experiment_context,
            manifest_evaluation_role=(
                "FROZEN_TEST" if experiment_context is not None else None
            ),
        )
    return benchmarks
...
def _validate_reference_context(value: Any) -> dict[str, Any]:
    _require_native(value, "$.reproducibility_context")
    if type(value) is not dict or set(value) != {
        ...
        raise SyntheticStrategyReportBundleError("reference context shape mismatch")
    if (
        value["schema_version"] != REFERENCE_CONTEXT_SCHEMA_VERSION
        or type(value["git_commit_sha"]) is not str
        or len(value["git_commit_sha"]) != 40
        or type(value["git_worktree_clean"]) is not bool
        or value["git_worktree_clean"] is not False
        or type(value["dependency_lock_hash"]) is not str
        or len(value["dependency_lock_hash"]) != 64
        or value["dependency_lock_fully_pinned"] is not True
        or value["dependency_lock_name"] != "requirements.research.lock"
        or type(value["runtime_version"]) is not str
        or not value["runtime_version"]
    ):
        raise SyntheticStrategyReportBundleError("reference context semantics invalid")
    return deepcopy(value)


def build_synthetic_strategy_report_bundle_v2(
    *,
    execute: bool = False,
    ...
    context = _validate_reference_context(reproducibility_context)
    plan = plan_synthetic_strategy_report_bundle_v1()
    frames, fixture = _build_fixture()
    inventory = build_current_strategy_family_inventory()
    if inventory["registered_strategy_ids"] != list(_STRATEGY_IDS):
    ...
    bundle = {
        ...
        "runtime_mutations": False,
        "gaps": [
            gap for gap in _MANDATORY_GAPS if gap != "DEPENDENCY_LOCK_NOT_BOUND"
        ],
        "status": "BLOCK",
        "maturity": "SYNTHETIC_BASELINE_ONLY",
        "authority": _authority(),
...
def _v1_projection_from_v2(bundle: dict[str, Any]) -> dict[str, Any]:
    projection = deepcopy(bundle)
    projection.pop("reproducibility_context", None)
    projection["schema_version"] = SCHEMA_VERSION
    projection["gaps"] = list(_MANDATORY_GAPS)
    projection["bundle_sha256"] = canonical_sha256({
        key: value for key, value in projection.items() if key != "bundle_sha256"
    })
    return projection


def _all_bundle_runs(bundle: dict[str, Any]) -> list[dict[str, Any]]:
    return [
...
def verify_synthetic_strategy_report_bundle_v2(bundle: dict[str, Any]) -> dict[str, Any]:
    try:
        _require_native(bundle)
        if type(bundle) is not dict or bundle.get("schema_version") != REFERENCE_SCHEMA_VERSION:
            raise SyntheticStrategyReportBundleError("reference bundle schema mismatch")
        context = _validate_reference_context(bundle.get("reproducibility_context"))
        _verify_bundle(_v1_projection_from_v2(bundle))
        if "DEPENDENCY_LOCK_NOT_BOUND" in bundle["gaps"]:
            raise SyntheticStrategyReportBundleError("dependency lock gap was not replaced")
        protocol_hash = bundle["fixture"]["partition_protocol"]["protocol_sha256"]
        runs = _all_bundle_runs(bundle)
        if len(runs) != 32:
            raise SyntheticStrategyReportBundleError("reference run count mismatch")
        role_map = {
            "TRAIN_BASELINE": "TRAIN",
            "VALIDATION_BASELINE": "VALIDATION",
            "FROZEN_COST_1X": "FROZEN_TEST",
        ...
        for run in runs:
            ...
            if (
                ...
                != role_map[run["evaluation_role"]]
                or manifest.get("evaluation_protocol_hash") != protocol_hash
                or manifest.get("evaluation_protocol_verified") is not True
                or "git_worktree_not_clean" not in manifest.get("blockers", [])
            ):
                raise SyntheticStrategyReportBundleError(
                    f"nested reproducibility binding invalid: {run['run_id']}"
                )
        _verify_seal(bundle, "bundle_sha256", "$")
    except Exception as exc:
        return {
            "status": "BLOCK",
            "blockers": [f"REFERENCE_BUNDLE_VERIFICATION_FAILED:{type(exc).__name__}:{exc}"],
            "authority": _authority(),
        }
    return {
        "status": "PASS",
...
def replay_synthetic_strategy_report_bundle_v2(bundle: dict[str, Any]) -> dict[str, Any]:
    structural = verify_synthetic_strategy_report_bundle_v2(bundle)
    if structural["status"] != "PASS":
        return structural
    replayed = build_synthetic_strategy_report_bundle_v2(
        execute=True,
        reproducibility_context=bundle["reproducibility_context"],
    )
    if replayed != bundle:
        return {
            "status": "BLOCK",
            "blockers": ["REFERENCE_BUNDLE_DETERMINISTIC_REPLAY_MISMATCH"],
            "authority": _authority(),
        }
    return {
        "status": "PASS",
...
def render_synthetic_strategy_report_bundle_markdown_v2(
    bundle: dict[str, Any],
) -> str:
    receipt = verify_synthetic_strategy_report_bundle_v2(bundle)
    if receipt["status"] != "PASS":
        raise SyntheticStrategyReportBundleError("refusing to render unverified reference bundle")
    markdown = render_synthetic_strategy_report_bundle_markdown_v1(
        _v1_projection_from_v2(bundle)
    )
    markdown = markdown.replace("- `DEPENDENCY_LOCK_NOT_BOUND`\n", "")
    marker = "> Every number below is SYNTHETIC_OBSERVATION_ONLY. It is not evidence of real-market performance or permission to trade."
    binding = (
        marker
        + "\n\n- Nested dependency lock bound: `true`"
        + f"\n- Dependency lock SHA-256: `{bundle['reproducibility_context']['dependency_lock_hash']}`"
        + "\n- Nested Git worktree clean: `false`"
    )
    markdown = markdown.replace(marker, binding)
    return markdown
