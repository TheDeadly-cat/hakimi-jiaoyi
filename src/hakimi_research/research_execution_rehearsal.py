...
from hakimi_research.research_order_lifecycle_contract import (
    MAX_IDEMPOTENCY_KEY_LENGTH,
    ORDER_TYPES,
    PAPER_TERMINAL_STATES,
    validate_research_rehearsal_lifecycle_order,
)
from hakimi_research.terminal_utils import pct
...
def research_execution_report(
    ...
    if type(symbol) is not str:
        raise TypeError("research_rehearsal_symbol_exact_str_required")
    if type(side) is not str:
        raise TypeError("research_rehearsal_side_exact_str_required")
    if type(order_type) is not str:
        raise TypeError("research_rehearsal_order_type_exact_str_required")
    for field_name, value in (
        ...
        if type(value) not in (int, float):
            raise TypeError(f"research_rehearsal_{field_name}_exact_number_required")
        if not math.isfinite(value):
            raise TypeError(f"research_rehearsal_{field_name}_finite_required")
    ...
    rows: list[Any] = []
    try:
        reader = book_reader or _empty_book_reader
    ...
    try:
        funding_reader = funding_rate_reader or _zero_funding_rate_reader
...
RESEARCH_EXECUTION_REHEARSAL_VERSION = "research-execution-rehearsal-v1"
LEGACY_REPLAY_WIRE_SCHEMA = "paper-lifecycle-v1"
RESEARCH_REHEARSAL_MODE = "RESEARCH_REHEARSAL"
ALLOW_PAPER = False
ALLOW_LIVE = False
ALLOW_ORDER_ENTRY = False

def _require_exact_native_tree(value: object, path: str) -> None:
    value_type = type(value)
    if value is None or value_type in (str, int, float, bool):
        if value_type is float and not math.isfinite(value):
            raise TypeError(f"research_rehearsal_non_finite:{path}")
        return
    if value_type is list:
        for index, item in enumerate(value):
            _require_exact_native_tree(item, f"{path}[{index}]")
        return
    if value_type is dict:
        for key, item in value.items():
            if type(key) is not str:
                raise TypeError(f"research_rehearsal_key_type:{path}")
            _require_exact_native_tree(item, f"{path}.{key}")
        return
    raise TypeError(f"research_rehearsal_exact_native_required:{path}")

def _default_instance_nonce(account_id: str) -> str:
    if type(account_id) is not str:
        raise TypeError("research_rehearsal_account_id_exact_str_required")
    payload = f"{RESEARCH_EXECUTION_REHEARSAL_VERSION}:{account_id}".encode("utf-8")
    return hashlib.sha256(payload).hexdigest()[:16]

def _deterministic_idempotency_key(request: dict[str, object], instance_nonce: str) -> str:
    payload = json.dumps(
        {"request": request, "instance_nonce": instance_nonce},
        ensure_ascii=True,
        sort_keys=True,
        separators=(",", ":"),
        allow_nan=False,
    ).encode("utf-8")
    return "research-rehearsal-" + hashlib.sha256(payload).hexdigest()

def _empty_book_reader(*args: object, **kwargs: object) -> list[object]:
    return []

def _zero_funding_rate_reader(*args: object, **kwargs: object) -> float:
    return 0.0

class ResearchExecutionRehearsalSimulator:
    ...
    def __init__(
        ...
        *,
        ...
        if type(account_id) is not str:
            raise TypeError("research_rehearsal_account_id_exact_str_required")
        if instance_nonce is not None and type(instance_nonce) is not str:
            raise TypeError("research_rehearsal_instance_nonce_exact_str_required")
        if type(max_orders) is not int:
            raise TypeError("research_rehearsal_max_orders_exact_int_required")
        self.now_ms = now_ms
        ...
        self._instance_nonce = (
            ...
            else _default_instance_nonce(account_id)
        )
    ...
    def _restore(self) -> None:
        ...
        try:
            ...
            self._restore_blockers = [f"research_rehearsal_history_load_failed:{type(exc).__name__}"]
            self._restore_error = type(exc).__name__
            return
        if not isinstance(rows, (list, tuple)):
            self._restore_status = "BLOCK"
            self._restore_blockers = ["research_rehearsal_history_invalid_type"]
            self._restore_error = type(rows).__name__
            return
        try:
            for row in rows[-self.max_orders:]:
                if not isinstance(row, dict):
                    raise ValueError("research_rehearsal_history_row_invalid")
                nested_order = row.get("order")
                order = deepcopy(nested_order if isinstance(nested_order, dict) else row)
                validate_research_rehearsal_lifecycle_order(order)
                ...
                if existing_id and existing_id != order_id:
                    raise ValueError("research_rehearsal_history_idempotency_conflict")
                ...
                if existing_risk_order_id and existing_risk_order_id != order_id:
                    raise ValueError("research_rehearsal_history_risk_request_conflict")
            ...
            self._sequence = 0
            self._restore_status = "BLOCK"
            if isinstance(exc, ValueError) and str(exc).startswith("research_rehearsal_history_"):
                blocker = str(exc)
            elif isinstance(exc, ValueError) and str(exc).startswith(
                ("research_rehearsal_contract_", "research_order_lifecycle_contract_")
            ):
                blocker = f"research_rehearsal_history_{exc}"
            else:
                blocker = f"research_rehearsal_history_restore_failed:{type(exc).__name__}"
    ...
    def _next_order_id(self) -> str:
        while True:
            self._sequence += 1
            order_id = f"research-rehearsal-{self.now_ms()}-{self._instance_nonce}-{self._sequence:06d}"
    ...
    def _report_from_order(
        ...
        *,
        ...
        if not isinstance(raw_report, dict):
            raise ValueError("research_order_lifecycle_contract_execution_report_invalid")
    ...
    def _resolve_persistence_race(self, order: dict[str, Any], error: Exception) -> dict[str, Any] | None:
        ...
        try:
            ...
            validate_research_rehearsal_lifecycle_order(winner)
        except Exception:
            return None
    ...
    def _risk_authorization_blockers(
        ...
        *,
        ...
        if type(risk_result) is not dict:
            return ["research_rehearsal_authorization_exact_dict_required"]
        blockers: list[str] = []
        ...
        if risk_result.get("allowed") is not True:
            blockers.append("research_rehearsal_risk_not_allowed")
        if risk_result.get("research_rehearsal_allowed") is not True:
            blockers.append("research_rehearsal_capability_not_allowed")
        if risk_result.get("paper_order_allowed") is not False:
            blockers.append("paper_order_boundary_invalid")
        if risk_result.get("live_order_allowed") is not False:
            blockers.append("live_order_boundary_invalid")
        if risk_result.get("order_entry_allowed") is not False:
            blockers.append("order_entry_boundary_invalid")
        if risk_result.get("mode") != RESEARCH_REHEARSAL_MODE:
            blockers.append("research_rehearsal_mode_required")
        risk_symbol = risk_result.get("symbol")
        if type(risk_symbol) is not str or risk_symbol.upper() != symbol.upper():
            blockers.append("risk_symbol_mismatch")
        risk_side = risk_result.get("side")
        if type(risk_side) is not str or risk_side.upper() != side.upper():
        ...
        if type(raw_context_order_type) is not str or raw_context_order_type.upper() != order_type.upper():
        ...
        if type(raw_context_idempotency) is not str or raw_context_idempotency.strip() != idempotency_key:
        ...
        *,
        ...
        for field_name, value in (
            ...
            if type(value) is not str:
                raise TypeError(f"research_rehearsal_{field_name}_exact_str_required")
        for field_name, value in (
            ("mark_price", mark_price),
            ("notional", notional),
            ("limit_price", limit_price),
        ...
        if risk_result is not None:
            if type(risk_result) is not dict:
                raise TypeError("research_rehearsal_risk_exact_dict_required")
            _require_exact_native_tree(risk_result, "risk_result")
        ...
        if context is not None:
            _require_exact_native_tree(context, "context")
        context = dict(context or {})
        if context.get("idempotency_key") in (None, ""):
            context["idempotency_key"] = _deterministic_idempotency_key(
                {
                    "symbol": symbol,
                    "side": side,
                    "order_type": order_type,
        ...
        if risk_result is not None and type(risk_result.get("context")) is dict:
            risk_result = dict(risk_result)
            bound_risk_context = dict(risk_result["context"])
            if bound_risk_context.get("idempotency_key") in (None, ""):
                bound_risk_context["idempotency_key"] = context["idempotency_key"]
            risk_result["context"] = bound_risk_context
        ...
        with self._lock:
                ...
                if existing is None and self.idempotency_loader:
                    try:
                        ...
                        if loaded is not None:
                            validate_research_rehearsal_lifecycle_order(loaded)
            ...
            if not existing_risk_order_id and self.risk_request_loader:
                ...
                if durable_risk_order is not None:
                    try:
                        validate_research_rehearsal_lifecycle_order(durable_risk_order)
                    except Exception:
                        return self._authorization_rejected(
                            side=side,
                            order_type=clean_order_type,
            ...
            lineage_context = {**clean_context, **risk_context}
            if lineage_context.get("idempotency_key") in (None, ""):
                lineage_context["idempotency_key"] = _deterministic_idempotency_key(
                    {
                        "symbol": symbol,
                        "side": side,
                        "order_type": order_type,
    ...
    def snapshot(self) -> dict[str, object]:
        ...
        if type(snapshot) is not dict:
            raise TypeError("research_rehearsal_snapshot_exact_dict_required")
        result = dict(snapshot)
        result.update(
            {
                "mode": RESEARCH_REHEARSAL_MODE,
                "research_only": True,
                "paper_allowed": False,
                "live_order_allowed": False,
