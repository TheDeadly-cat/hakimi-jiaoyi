...
ARCHIVED_PAPER_READ_ONLY_PATHS = frozenset({
    "/api/paper/ledger",
...
_ARCHIVED_EXECUTION_BLOCKED_PATHS = frozenset({"/api/order/estimate"})


def archived_execution_route_state(method: str, path: str) -> str:
    if type(method) is not str or type(path) is not str:
        raise TypeError("archived_execution_route_state requires exact native strings")
    if path in _ARCHIVED_EXECUTION_BLOCKED_PATHS:
        return "BLOCK"
    if not path.startswith("/api/paper/"):
        return "NOT_APPLICABLE"
    catalog = build_product_capability_catalog().to_dict()
    if catalog.get("capabilities", {}).get("paper_execution") != "Archived":
        return "BLOCK"
    if method == "GET" and path in ARCHIVED_PAPER_READ_ONLY_PATHS:
        return "READ_ONLY"
    return "BLOCK"
