...
from exchange_terminal.application import (
    strategy_correlation_incumbent_snapshot_replay_cursor_provider_registration_challenge_clock_attestation_binding_v1 as binding,
)
from exchange_terminal.application import (
...
def _der_spki(private_key: Ed25519PrivateKey) -> bytes:
...
class ReplayCursorProviderRegistrationChallengeClockBindingV1Tests(unittest.TestCase):
    def setUp(self) -> None:
        provider_private = Ed25519PrivateKey.generate()
        provider_spki = _der_spki(provider_private)
        self.provider_kwargs = {
            "registry_id": "synthetic.replay.registry.v1",
            "operator_identity_claim": "synthetic.provider.operator.v1",
            "public_key_spki_sha256": sha256(provider_spki).hexdigest(),
        ...
        self.challenge_spki = _der_spki(self.challenge_private)
        ...
        self.registration_nonce_hash = sha256(b"registration-nonce").hexdigest()
        self.challenge_build_kwargs = {
            ...
            "challenge_id_hash": sha256(b"challenge-id").hexdigest(),
        ...
        self.signed_challenge = challenge.build_signed_replay_cursor_provider_registration_challenge_v1(
            self.challenge,
            self.provider_preregistration,
            self.authority_preregistration,
        ...
        self.challenge_evidence = (
            challenge.evaluate_signed_replay_cursor_provider_registration_challenge_v1(
        ...
        self.clock_private = [Ed25519PrivateKey.generate(), Ed25519PrivateKey.generate()]
        self.clock_raw = [_raw_public(key) for key in self.clock_private]
        ...
        self.clock_registration = clock.build_trusted_clock_authority_registration_v3(
            self.clock_authorities,
            minimum_sources=2,
        ...
        self.clock_receipts, self.expected_receipt_hashes, self.clock_attestation = (
    ...
    def build_clock_chain(
        ...
        *,
        ...
        attestation = clock.evaluate_trusted_clock_authority_v3(
            ...
            receipts,
            self.clock_public_keys,
            expected_registration_hash=self.clock_registration["registration_hash"],
    ...
    def evaluate(self, **overrides):
        ...
        return binding.evaluate_replay_cursor_provider_registration_challenge_clock_binding_v1(
            **kwargs
        )
    ...
    def test_nonce_must_equal_registration_nonce(self) -> None:
        receipts, expected, attestation = self.build_clock_chain(
            nonce_hash=sha256(b"wrong-nonce").hexdigest()
        )
        evidence = self.evaluate(
            clock_attestation=attestation,
            clock_receipts=receipts,
    ...
    def test_reference_time_outside_declared_window_is_blocked(self) -> None:
        receipts, expected, attestation = self.build_clock_chain(
            observed=(1_013_000, 1_013_020), verification_time_ms=1_013_500
        )
        evidence = self.evaluate(
            clock_attestation=attestation,
            clock_receipts=receipts,
    ...
    def test_challenge_evidence_promotion_is_rejected(self) -> None:
        forged = deepcopy(self.challenge_evidence)
        forged["facts"]["challenge_freshness_verified"] = True
        evidence = self.evaluate(challenge_evidence=forged)
        self.assertEqual(evidence["status"], "BLOCK")
        self.assertFalse(evidence["facts"]["challenge_evidence_exact"])
    ...
    def test_expected_hash_drift_is_blocked(self) -> None:
        ...
        self.assertEqual(
            self.evaluate(expected_challenge_evidence_hash="1" * 64)["status"],
            "BLOCK",
        )
    ...
    def test_public_verifier_rebuilds_and_rejects_mutation(self) -> None:
        ...
        self.assertTrue(
            binding.verify_replay_cursor_provider_registration_challenge_clock_binding_evidence_v1(
                evidence,
                *args,
    ...
    def test_evidence_redacts_receipts_keys_and_signatures(self) -> None:
        ...
        self.assertNotIn(_b64(self.challenge_spki), encoded)
        self.assertNotIn(_b64(self.challenge_signature), encoded)
