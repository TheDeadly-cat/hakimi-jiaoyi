...
class ChallengeConsumptionProviderSignedRegistrationV1Tests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.preregistration_kwargs = {
            ...
            "provider_implementation_claim_sha256": sha256(
                b"synthetic-consumption-provider"
            ).hexdigest(),
        }
        self.prereg = preregistration.build_challenge_consumption_provider_preregistration_v1(
            **self.preregistration_kwargs
        )
        self.challenge_hash = sha256(b"synthetic-provider-registration-challenge").hexdigest()
        self.nonce_hash = sha256(b"synthetic-provider-registration-nonce").hexdigest()
        self.claim_kwargs = {
        ...
        self.claim = registration.build_challenge_consumption_provider_registration_claim_v1(
            self.prereg, **self.claim_kwargs
        )
        self.signature = self.private.sign(bytes.fromhex(self.claim["claim_hash"]))
        self.signed = registration.build_signed_challenge_consumption_provider_registration_v1(
            self.claim,
            self.prereg,
    ...
    def evaluate(self, signed=None, claim=None, **overrides):
        ...
        return registration.evaluate_signed_challenge_consumption_provider_registration_v1(
    ...
    def test_claim_is_exact_deterministic_and_operationally_blocked(self) -> None:
        rebuilt = registration.build_challenge_consumption_provider_registration_claim_v1(
            self.prereg, **self.claim_kwargs
        )
        self.assertEqual(rebuilt, self.claim)
        self.assertEqual(rebuilt["status"], "BLOCKED")
        self.assertTrue(
            registration.verify_challenge_consumption_provider_registration_claim_v1(
                rebuilt,
                self.prereg,
    ...
    def test_wrong_key_self_signature_fails_preregistered_binding(self) -> None:
        ...
        wrong_signed = registration.build_signed_challenge_consumption_provider_registration_v1(
            self.claim,
            self.prereg,
    ...
    def test_tampered_signature_is_rejected(self) -> None:
        ...
        tampered = registration.build_signed_challenge_consumption_provider_registration_v1(
            self.claim,
            self.prereg,
            public_key_spki_base64=_b64(self.spki),
    ...
    def test_preregistration_drift_cannot_build_claim(self) -> None:
        ...
        with self.assertRaises(
            registration.ChallengeConsumptionProviderSignedRegistrationError
        ):
    ...
    def test_evidence_verifier_rebuilds_and_rejects_mutation(self) -> None:
        ...
        self.assertTrue(
            registration.verify_signed_challenge_consumption_provider_registration_evidence_v1(
                evidence,
                self.signed,
                self.claim,
                self.prereg,
