...
from exchange_terminal.services import (
    strategy_correlation_matrix_geometry_budget_multi_window_presentation_request_scope_source_resolver_preregistration_v1
    as subject,
)
from exchange_terminal.services import (
    strategy_correlation_matrix_geometry_budget_multi_window_presentation_trusted_internal_provider_preregistration_v1
    as provider_preregistration,
)
...
class GeometryBudgetMultiWindowRequestScopeResolverPreregistrationV1Tests(
    unittest.TestCase
):
    def setUp(self) -> None:
        self.document = subject.build_strategy_correlation_matrix_geometry_budget_multi_window_presentation_request_scope_source_resolver_preregistration_v1()
    ...
    def test_schema_state_and_hash_are_exact(self) -> None:
        ...
        self.assertEqual(
            len(self.document["scope_resolver_preregistration_hash"]), 64
        )

    def test_exact_adr0337_predecessor_is_pinned_and_unbound(self) -> None:
        predecessor = provider_preregistration.build_strategy_correlation_matrix_geometry_budget_multi_window_presentation_trusted_internal_provider_preregistration_v1()
        self.assertTrue(
            provider_preregistration.verify_strategy_correlation_matrix_geometry_budget_multi_window_presentation_trusted_internal_provider_preregistration_v1(
                predecessor
            )
        )
        self.assertEqual(
        ...
        self.assertEqual(
            predecessor["provider_preregistration_hash"],
            self.document["predecessor_contract"]["provider_preregistration_hash"],
        )
    ...
    def test_predecessor_source_files_match_pins(self) -> None:
        ...
        for prefix in ("provider_implementation", "provider_test", "provider_adr"):
    ...
    def test_request_scope_evidence_is_hash_only_server_owned_and_unregistered(
        self,
    ) -> None:
        contract = self.document["request_scope_contract"]
        self.assertEqual(
    ...
    def test_source_resolver_contract_is_exact_explicit_and_side_effect_free(
        self,
    ) -> None:
        ...
        self.assertEqual(
            ...
            "EXPLICIT_ADR0334_INTERNAL_GEOMETRY_BOUND_CHAIN_ONLY",
        )
        for field in (
    ...
    def test_source_resolver_requires_scope_generation_and_single_use(self) -> None:
        ...
        self.assertEqual(
            contract["input_provider_output_shape_hash"],
            provider_preregistration.PROVIDER_OUTPUT_SHAPE_HASH,
        )
        self.assertEqual(
            contract["input_request_role_hash"],
            provider_preregistration.REQUEST_ROLE_HASH,
        )
        self.assertEqual(
            contract["input_verification_context_role_hash"],
            provider_preregistration.VERIFICATION_CONTEXT_ROLE_HASH,
        )

    def test_cross_binding_contract_pins_all_boundaries(self) -> None:
        ...
        self.assertEqual(
            contract["provider_preregistration_hash"],
            subject.PREDECESSOR_CONTRACT["provider_preregistration_hash"],
        )
        self.assertEqual(
            contract["candidate_contract_hash"],
            subject.PREDECESSOR_CONTRACT["candidate_contract_hash"],
        )

    def test_cross_binding_requires_receipts_same_scope_and_unconsumed_state(
        self,
    ) -> None:
        contract = self.document["cross_binding_contract"]
        for field in (
    ...
    def test_facts_record_no_producer_resolver_provider_handler_or_route(
        self,
    ) -> None:
    ...
    def test_builder_accepts_no_runtime_or_policy_override(self) -> None:
        function = subject.build_strategy_correlation_matrix_geometry_budget_multi_window_presentation_request_scope_source_resolver_preregistration_v1
    ...
    def test_exact_rebuild_is_deterministic_and_verifiable(self) -> None:
        rebuilt = subject.build_strategy_correlation_matrix_geometry_budget_multi_window_presentation_request_scope_source_resolver_preregistration_v1()
        self.assertEqual(rebuilt, self.document)
        self.assertTrue(
            subject.verify_strategy_correlation_matrix_geometry_budget_multi_window_presentation_request_scope_source_resolver_preregistration_v1(
                self.document
            )
        )
    ...
    def test_public_api_has_no_host_runtime_database_cache_or_secret_parameter(
        ...
        for function in (
            subject.build_strategy_correlation_matrix_geometry_budget_multi_window_presentation_request_scope_source_resolver_preregistration_v1,
            subject.verify_strategy_correlation_matrix_geometry_budget_multi_window_presentation_request_scope_source_resolver_preregistration_v1,
        ):
