...
import exchange_terminal.application.synthetic_strategy_deflated_sharpe_validation_v1 as compatibility
import hakimi_research.synthetic_strategy_deflated_sharpe_validation as canonical
...
WRAPPER_PATH = (
    ...
    / "exchange_terminal"
    / "application"
    / "synthetic_strategy_deflated_sharpe_validation_v1.py"
)
ARCHIVE_PATH = (
    REPO_ROOT
    / "archive"
    / "historical_research"
    / "adr0575_synthetic_strategy_deflated_sharpe_validation_v1.py"
)
...
def _reseal(record: dict, field: str = "bundle_sha256") -> None:
    record[field] = canonical_trial_return_matrix_sha256(
        {key: value for key, value in record.items() if key != field}
    )


class SyntheticStrategyDeflatedSharpeValidationV2Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.plan = (
            canonical.plan_synthetic_strategy_deflated_sharpe_validation_v2()
        )
        cls.bundle = (
            canonical.build_synthetic_strategy_deflated_sharpe_validation_v2(
                cls.matrix,
                execute=True,
            )
        )

    def test_01_plan_binds_v2_matrix_and_zero_runs(self) -> None:
        self.assertEqual(
        ...
        self.assertEqual(
            self.plan["expected_matrix_dependency_bound_run_count"],
            18,
        )

    def test_02_bundle_verifies_provenance_and_six_diagnostics(self) -> None:
        receipt = (
            canonical.verify_synthetic_strategy_deflated_sharpe_validation_v2(
                self.bundle
            )
        )
    ...
    def test_03_diagnostics_remain_descriptive_and_bounded(self) -> None:
        ...
        self.assertFalse(
            self.plan["policy"]["formal_inference_claimed"]
        )
        self.assertIsNone(self.plan["policy"]["decision_threshold"])

    def test_04_v1_consumer_rejects_v2_matrix_before_analysis(self) -> None:
        with self.assertRaises(
            canonical.SyntheticStrategyDeflatedSharpeValidationError
        ):
            canonical.build_synthetic_strategy_deflated_sharpe_validation_v1(
                self.matrix,
                execute=True,
            )

    def test_05_context_count_and_authority_tamper_fail_closed(self) -> None:
        mutations = (
    ...
    def test_06_exact_native_wrapper_and_archive_boundaries(self) -> None:
        ...
        self.assertIs(
            compatibility.build_synthetic_strategy_deflated_sharpe_validation_v1,
            canonical.build_synthetic_strategy_deflated_sharpe_validation_v1,
        )
        self.assertIs(
            compatibility.build_synthetic_strategy_deflated_sharpe_validation_v2,
            canonical.build_synthetic_strategy_deflated_sharpe_validation_v2,
        )
        ...
        self.assertEqual(
            ...
            "b202f3dd5e4a72a6ae2f17704bab26fb77b4c323a223af9921bd34e82cf6ad9e",
        )

    def test_07_replay_is_exact_and_executes_zero_backtests(self) -> None:
        receipt = (
            canonical.replay_synthetic_strategy_deflated_sharpe_validation_v2(
                self.bundle
            )
        )
    ...
    def test_08_renderer_is_neutral_and_retains_pbo_gap(self) -> None:
        markdown = canonical.render_synthetic_strategy_deflated_sharpe_validation_markdown_v2(
            self.bundle
        )
        for heading in ("## SOURCE", "## GAP", "## MATURITY", "## PERMISSION"):
            self.assertIn(heading, markdown)
        self.assertIn("PROBABILITY_OF_BACKTEST_OVERFITTING_GAP", markdown)
