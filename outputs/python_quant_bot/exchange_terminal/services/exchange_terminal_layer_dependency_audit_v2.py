"""Pure AST audit for nested exchange_terminal layer dependency direction.
...
SCHEMA_VERSION = "exchange-terminal-layer-dependency-audit-v2"
STATIC_FINGERPRINT = (
    "20260824-exchange-terminal-layer-dependency-audit-v2-"
    "nested-module-pure-ast-partial-layering-lock-1"
)
...
_MAX_MODULE_NAME_LENGTH = 512
_MAX_MODULE_SEGMENTS = 16
_MAX_MODULES = 512
...
def _module_name_is_valid(module: str) -> bool:
    parts = module.split(".")
    return (
        len(module) <= _MAX_MODULE_NAME_LENGTH
        and 3 <= len(parts) <= _MAX_MODULE_SEGMENTS
        and parts[0] == "exchange_terminal"
        and parts[1] in LAYERS
        and all(
            _MODULE_SEGMENT_RE.fullmatch(segment) is not None
...
def _snapshot_sources(module_sources: Any) -> dict[str, str] | None:
    ...
    for module, source in module_sources.items():
        if (
            ...
            or not _module_name_is_valid(module)
...
def evaluate_exchange_terminal_layer_dependency_audit_v2(
    module_sources: Any,
) -> dict[str, Any]:
...
def verify_exchange_terminal_layer_dependency_audit_v2(
    document: Any,
    module_sources: Any,
) -> bool:
    try:
        if type(document) is not dict:
            return False
        expected = evaluate_exchange_terminal_layer_dependency_audit_v2(
            module_sources
        )
    except Exception:
        return False
...
__all__ = [
    ...
    "STATUS_BLOCKED",
    "STATUS_CONFORMING",
    "evaluate_exchange_terminal_layer_dependency_audit_v2",
    "verify_exchange_terminal_layer_dependency_audit_v2",
]
