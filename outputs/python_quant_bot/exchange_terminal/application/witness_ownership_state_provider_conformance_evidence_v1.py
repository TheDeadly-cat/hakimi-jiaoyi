"""Signed observer-report quorum candidate for ADR0414."""
...
from exchange_terminal.application import (
    witness_ownership_state_provider_conformance_plan_v1 as conformance_plan,
)
from exchange_terminal.application import (
...
QUORUM_EVIDENCE_SCHEMA_VERSION = (
    "witness-ownership-provider-conformance-observer-quorum-evidence-v1"
)
STATIC_FINGERPRINT = (
    "20260824-witness-ownership-provider-observer-quorum-v1-lock-1"
)
...
SIGNATURE_DOMAIN = (
    "hakimi.strategy-correlation.witness-ownership.provider-conformance-observer.v1"
)
...
def _validate_upstreams(
    ...
    *,
    ...
    if not conformance_plan.verify_witness_ownership_state_provider_conformance_plan_v1(
        plan_document,
    ...
    if not signed_receipt.verify_signed_witness_ownership_state_provider_receipt_evidence_v1(
    ...
    if (
        ...
        or signed_receipt_evidence_document.get("admission_status") != "BLOCKED"
        or signed_receipt_evidence_document.get("facts", {}).get(
...
def build_witness_ownership_provider_conformance_observer_report_v1(
    plan_document: Any,
    provider_preregistration_document: Any,
    signed_receipt_evidence_document: Any,
    *,
...
def verify_witness_ownership_provider_conformance_observer_report_v1(
    report_document: Any,
    plan_document: Any,
    provider_preregistration_document: Any,
    signed_receipt_evidence_document: Any,
    **build_kwargs: Any,
    ...
    if type(report_document) is not dict:
        return False
    try:
        expected = build_witness_ownership_provider_conformance_observer_report_v1(
            plan_document,
            provider_preregistration_document,
            signed_receipt_evidence_document,
...
def build_witness_ownership_provider_conformance_observer_signature_message_hash_v1(
    report_document: Any,
    plan_document: Any,
    signed_receipt_evidence_document: Any,
) -> str:
    if (
        ...
        or not _is_hash(
            signed_receipt_evidence_document.get("verification_evidence_hash")
        )
    ):
    ...
    return strict_canonical_hash(
        {
            ...
            "conformance_plan_hash": plan_document["conformance_plan_hash"],
            "signed_receipt_evidence_hash": signed_receipt_evidence_document[
...
def build_signed_witness_ownership_provider_conformance_observer_report_v1(
    report_document: Any,
    plan_document: Any,
    provider_preregistration_document: Any,
    signed_receipt_evidence_document: Any,
    *,
    ...
    if not verify_witness_ownership_provider_conformance_observer_report_v1(
        report_document,
        plan_document,
        provider_preregistration_document,
        signed_receipt_evidence_document,
    ...
    message_hash = build_witness_ownership_provider_conformance_observer_signature_message_hash_v1(
        report_document,
        plan_document,
        signed_receipt_evidence_document,
    )
    ...
    return seal_strict_canonical_document(body, "signed_observer_report_hash")


def evaluate_witness_ownership_provider_conformance_observer_quorum_v1(
    ...
    *,
    ...
    for signed_document in rows:
        ...
        try:
            ...
            report_exact = verify_witness_ownership_provider_conformance_observer_report_v1(
                report,
                plan_document,
                provider_preregistration_document,
                signed_receipt_evidence_document,
            ...
            expected_signed = build_signed_witness_ownership_provider_conformance_observer_report_v1(
                report,
                plan_document,
                provider_preregistration_document,
                signed_receipt_evidence_document,
            ...
            signed_report_hash = expected_signed["signed_observer_report_hash"]
            ...
            signature = _decode_signature(signed_document["signature_base64"])
            registration = registrations[observer_id]
            key_hash_matches = (
                sha256(spki_bytes).hexdigest()
                == registration["public_key_spki_sha256"]
            )
            message_hash = expected_signed["signature_message_hash"]
            try:
    ...
    if not upstream_exact:
        dynamic_blockers.append("UPSTREAM_CHAIN_NOT_EXACT")
    ...
    body = {
        ...
        "static_fingerprint": STATIC_FINGERPRINT,
        "status": "PASS" if quorum_verified else "BLOCK",
        ...
        "decision": (
            "TWO_OF_THREE_SIGNED_CONFORMANCE_REPORTS_VERIFIED_EXTERNAL_"
            "EXECUTION_AND_SOURCE_TRUTH_STILL_BLOCKED"
            if quorum_verified
            else "OBSERVER_CONFORMANCE_REPORT_QUORUM_INVALID_OR_INCOMPLETE"
        ),
        "blockers": dynamic_blockers + list(_PERMANENT_BLOCKERS),
        "observer_results": observer_results,
        "quorum_summary": {
            "registered_observer_count": 3,
            "submitted_report_count": len(rows),
            "required_signature_quorum": 2,
            "passing_observer_ids": passing_ids,
            "duplicate_observer_ids_detected": duplicate_observer_ids,
        },
        "source": {
        ...
        "facts": {
            ...
            "signed_observer_report_quorum_verified": quorum_verified,
            "all_required_case_results_claimed_by_quorum": quorum_verified,
        ...
        "redaction": {
            ...
            "raw_test_transcripts_embedded": False,
        },
        "limitations": [
            "A signed observer-report quorum proves only local signatures over structurally complete report claims.",
            "Distinct IDs, keys, and organization-claim hashes do not prove observer identity or independence.",
            "No provider call, external test execution, durability, linearizability, rollback resistance, persistence, profitability, or trading authority is verified.",
        ],
    }
    return seal_strict_canonical_document(body, "quorum_evidence_hash")


def verify_witness_ownership_provider_conformance_observer_quorum_v1(
    document: Any,
    signed_report_documents: Any,
    plan_document: Any,
    provider_preregistration_document: Any,
    ...
    **evaluation_kwargs: Any,
    ...
    if type(document) is not dict or not _is_hash(
        expected_quorum_evidence_hash
    ):
        return False
    expected = evaluate_witness_ownership_provider_conformance_observer_quorum_v1(
...
__all__ = [
    ...
    "SIGNED_OBSERVER_REPORT_SCHEMA_VERSION",
    "STATIC_FINGERPRINT",
    "build_signed_witness_ownership_provider_conformance_observer_report_v1",
    "build_witness_ownership_provider_conformance_observer_report_v1",
    "build_witness_ownership_provider_conformance_observer_signature_message_hash_v1",
    "evaluate_witness_ownership_provider_conformance_observer_quorum_v1",
    "verify_witness_ownership_provider_conformance_observer_quorum_v1",
]
