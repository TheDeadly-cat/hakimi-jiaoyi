"""Unmounted replay-cursor provider conformance plan for ADR0479."""
...
STATIC_FINGERPRINT = (
    "20260825-replay-cursor-provider-conformance-plan-v1-lock-1"
)
...
SIGNED_RECEIPT_IMPLEMENTATION_SHA256 = (
    "8a273326eef4093c994295e91a3c1a63188afca825f85cd5319a2361394c9481"
)

EXPECTED_CASE_IDS = (
    "EXACT_ADVANCE_ACCEPTANCE",
    "UNSIGNED_ADVANCED_REJECTION",
    "COMMAND_HASH_REBINDING_REJECTION",
    "INTENT_HASH_REBINDING_REJECTION",
    "REGISTRY_ID_REBINDING_REJECTION",
    "BASE_CURSOR_CAS_CONFLICT",
    "PROPOSED_CURSOR_BINDING",
    "DUPLICATE_BEFORE_CONFLICT",
    "REJECTED_CURSOR_NO_ADVANCE",
    "RECEIPT_SCHEMA_EXACTNESS",
    "SIGNED_RECEIPT_BINDING",
    "TIMEOUT_AFTER_COMMIT_IDEMPOTENCY",
    "CONCURRENT_SAME_COMMAND_SINGLE_ADVANCE",
    "LINEARIZABLE_READ_AFTER_WRITE",
    "RESTART_CURSOR_RECOVERY",
    "ROLLBACK_REFUSAL",
...
def _normalize_observers(
    ...
    *,
    ...
    trust_domains = [row["trust_domain"] for row in normalized]
    if (
        len(set(observer_ids)) != 3
        or len(set(key_hashes)) != 3
        or len(set(organization_hashes)) != 3
        or len(set(trust_domains)) != 3
...
def expected_replay_cursor_provider_conformance_cases_v1() -> list[dict[str, Any]]:
    return [
        {
            "case_id": case_id,
            "required": True,
...
def build_replay_cursor_provider_conformance_plan_v1(
    ...
    **provider_preregistration_kwargs: Any,
    ...
    if not provider_preregistration.verify_replay_cursor_provider_preregistration_v1(
    ...
    identity = provider_preregistration_document["identity"]
    observers = _normalize_observers(
        observer_registrations,
        provider_public_key_spki_sha256=identity[
            "public_key_spki_sha256"
        ],
    )
    body = {
        ...
        "provider": {
            ...
            "public_key_spki_sha256": identity[
                "public_key_spki_sha256"
            ],
        ...
        "observers": observers,
        "cases": expected_replay_cursor_provider_conformance_cases_v1(),
        "target": {
            "command_schema_version": (
                provider_port.COMPARE_AND_ADVANCE_COMMAND_SCHEMA_VERSION
            ),
            "result_schema_version": (
                provider_port.COMPARE_AND_ADVANCE_RESULT_SCHEMA_VERSION
            ),
            "signed_receipt_schema_version": (
                signed_receipt.SIGNED_RECEIPT_SCHEMA_VERSION
            ),
            "signed_receipt_evidence_schema_version": (
                signed_receipt.VERIFICATION_EVIDENCE_SCHEMA_VERSION
            ),
        ...
        "source": {
            "signed_receipt_static_fingerprint": (
                signed_receipt.STATIC_FINGERPRINT
            ),
            "signed_receipt_implementation_sha256": (
                SIGNED_RECEIPT_IMPLEMENTATION_SHA256
            ),
...
__all__ = [
    ...
    "expected_replay_cursor_provider_conformance_cases_v1",
