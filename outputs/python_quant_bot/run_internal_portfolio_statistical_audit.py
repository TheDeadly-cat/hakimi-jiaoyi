...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run a deterministic benchmark-relative statistical audit for a frozen portfolio study."
    )
    ...
    if evidence_bundle_audit.get("status") != "PASS":
        ...
        return 2
    result = audit_portfolio_research_statistics(source, generated_at=int(time.time() * 1000))
    result.update({
    ...
    output = args.output.resolve() if args.output else (
        report_dir / time.strftime("portfolio_statistical_audit_%Y%m%d_%H%M%S.json")
    )
    ...
    print(json.dumps({
        ...
        "stage_summary": {
            stage: {
                ...
                "observation_count": audit.get("observation_count"),
                "compound_excess_return_pct": (audit.get("observed") or {}).get("compound_excess_return_pct"),
                "positive_probability": (audit.get("bootstrap") or {}).get("compound_excess_positive_probability"),
                "selection_adjusted_probability": (
                    audit.get("bootstrap") or {}
                ).get("selection_adjusted_compound_excess_probability"),
                "blockers": audit.get("blockers"),
            }
            for stage, audit in dict(result.get("stages") or {}).items()
        },
        "research_only": True,
        "paper_authorized": False,
        "live_order_allowed": False,
