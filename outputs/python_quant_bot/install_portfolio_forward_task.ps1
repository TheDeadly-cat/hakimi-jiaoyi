...
$TaskName = "HakimiTradeV2-PortfolioForwardObservation"
$ProjectRoot = Split-Path -Parent $MyInvocation.MyCommand.Path
$Runner = Join-Path $ProjectRoot "run_portfolio_forward_scheduler.py"
...
if (-not (Test-Path -LiteralPath $Runner)) {
    throw "Forward scheduler runner not found: $Runner"
}
...
Register-ScheduledTask `
    -TaskName $TaskName `
    -Description "Research-only portfolio forward observation. It cannot place paper or live orders." `
    -Action $Action `
    -Trigger $Trigger `
    -Settings $Settings `
    -Principal $Principal `
