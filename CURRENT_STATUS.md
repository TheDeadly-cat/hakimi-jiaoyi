...
更新：2026-09-13；运行统计截至北京时间17:41，精确时间及身份见链接回执。项目定位为研究软件，当前结果不支持自主实盘或盈利声明。
...
| 官方模拟 X1 | [独立适配候选](docs/futu-simulation-adapter.md)已实现，代码`8c387720…`通过26项针对性检查及原状态机25项回归；真实13次只读请求成功，两轮账户读取一致、AMD身份/1股单位/0.01美元步长及1美元限价现金容量已核对。费用及成交API受限；实际订单周期、费用/结算原件和正式接入仍未完成，未下单。此候选尚未合入main |
| 已部署运行件 | 固定bundle来源`bbf7cd2…`，新预检候选`d4afcca…`尚未部署。两项Windows任务已启用，旧heartbeat暂停 |
| 当前观察 O1 | 北京时间9月13日12:00—9月16日12:00，巡检至12:10。首轮FAILED后人工恢复LATE，随后五个自然小时ON_TIME，10份新策略记录独立重放通过。窗口未结束，两条通知积压且未确认显示 |
| 后续C/D | 财务字段人工核准未完成，内容策略未运行；日期过滤结果不代替字段核准 |

最新观察回执：[首轮失败保留及五个后续小时](docs/research-evidence/observation-activation-20260913/natural-hours-20260913.json)。维护者对PR #9的合并授权已执行；后续官方模拟下单仍须单独授权。
